...
type BlockchainMigrator struct {
	config       config.Database
	sqlSchemaDir string
}

func NewBlockchainMigrator(config config.Database, sqlSchemaDir string) *BlockchainMigrator {
	return &BlockchainMigrator{config: config, sqlSchemaDir: sqlSchemaDir}
}

func (m *BlockchainMigrator) Migrate(ctx context.Context, version int) error {
